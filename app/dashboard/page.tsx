...
const recentClients = [
  {
    name: "RC Plenitud Seguros",
    status: "Onboarding",
    note: "Conectar WhatsApp API oficial",
  },
  {
    name: "Power Tech",
    status: "Activo",
    note: "Optimizar flujo de seguimiento",
  },
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    Activo:
      "bg-emerald-500/15 text-emerald-200 ring-1 ring-emerald-400/30 shadow",
    Onboarding:
      "bg-amber-500/15 text-amber-200 ring-1 ring-amber-400/30 shadow",
    Prueba: "bg-sky-500/15 text-sky-200 ring-1 ring-sky-400/30 shadow",
  };
  const classes =
    styles[status] ??
    "bg-slate-400/10 text-slate-200 ring-1 ring-slate-500/30 shadow";
  return <span className={px-2 py-0.5 text-xs rounded-full ${classes}}>{status}</span>;
}
